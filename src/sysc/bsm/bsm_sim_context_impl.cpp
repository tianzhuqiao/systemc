...
bool bsm_sim_object_impl::ReadSignal(bsm_object_value * v) {
    ...
    if (BSM_CHECK_TYPE(BSM_REG_DOUBLE)) {
        ...
        assert(dyObj);
        snprintf(v->sValue, sizeof(v->sValue), "%s", dyObj->read().c_str());
        v->type = TYPE_STRING;
...
bsm_sim_trace_file_impl::bsm_sim_trace_file_impl(const char* name, int nType)
    :m_trace(NULL) {
    m_trace = static_cast<bsm_trace_file*>(sc_create_bsm_trace_file(name, nType));
    assert(m_trace);
}

bsm_sim_trace_file_impl::~bsm_sim_trace_file_impl() {
    if (m_trace) {
        sc_close_bsm_trace_file(m_trace);
        m_trace = NULL;
    }
}
...
bsm_sim_trace_buf_impl::bsm_sim_trace_buf_impl(const char* name)
    :m_trace(NULL) {
    m_trace = sc_create_bsm_trace_buf(name);
    assert(m_trace);
}

bsm_sim_trace_buf_impl::~bsm_sim_trace_buf_impl() {
    if (m_trace) {
        sc_close_bsm_trace_buf(m_trace);
        m_trace = NULL;
    }
}
...
bsm_sim_context* bsm_create_sim_context(sc_module* top) {
    return  new bsm_sim_context_impl(top);
}
